const fs = require('node:fs');

function loadSettings(filePath) {
  const raw = fs.readFileSync(filePath, 'utf8');
  return JSON.parse(raw);
}

function getProfile(settings, profileName, maxTicketsAllowed) {
  const profile = settings.profiles && settings.profiles[profileName];
  if (!profile) {
    throw new Error(`No profile named "${profileName}" in settings.json`);
  }
  if (!settings.buyer || !settings.buyer.name || !settings.buyer.phone) {
    throw new Error('settings.json buyer must include name and phone');
  }
  if (!profile.showUrl || !profile.sessionMatch) {
    throw new Error(`Profile "${profileName}" must include showUrl and sessionMatch`);
  }
  const ticketCount = profile.ticketCount || 1;
  if (typeof maxTicketsAllowed === 'number' && ticketCount > maxTicketsAllowed) {
    throw new Error(
      `Profile "${profileName}" requests ${ticketCount} tickets, ` +
        `but this show allows a maximum of ${maxTicketsAllowed}`
    );
  }
  return {
    showUrl: profile.showUrl,
    sessionMatch: profile.sessionMatch,
    sessionTime: profile.sessionTime || null,
    targetDatetime: profile.targetDatetime || null,
    ticketCount,
    preferredSeats: profile.preferredSeats || [],
    retryWindowSeconds: profile.retryWindowSeconds || 10,
    retryIntervalMs: profile.retryIntervalMs || 250,
    buyer: settings.buyer,
  };
}

const DEFAULT_TEST_MODE_STEP_DELAY_MS = 2000;

function getStepDelayMs(settings) {
  if (settings.testMode !== true) return 0;
  return typeof settings.testModeDelayMs === 'number'
    ? settings.testModeDelayMs
    : DEFAULT_TEST_MODE_STEP_DELAY_MS;
}

function getRecording(settings, name) {
  const recording = settings.recordings && settings.recordings[name];
  if (!recording) {
    throw new Error(`No recording named "${name}" in settings.json`);
  }
  if (!recording.startUrl || !Array.isArray(recording.actions)) {
    throw new Error(`Recording "${name}" must include startUrl and an actions array`);
  }
  return {
    startUrl: recording.startUrl,
    targetDatetime: recording.targetDatetime || null,
    retryWindowSeconds: recording.retryWindowSeconds || 10,
    retryIntervalMs: recording.retryIntervalMs || 250,
    actions: recording.actions,
  };
}

function saveRecording(settings, name, recording) {
  const next = {
    ...settings,
    recordings: { ...(settings.recordings || {}) },
  };
  next.recordings[name] = {
    startUrl: recording.startUrl,
    targetDatetime: recording.targetDatetime !== undefined ? recording.targetDatetime : null,
    retryWindowSeconds: recording.retryWindowSeconds || 10,
    retryIntervalMs: recording.retryIntervalMs || 250,
    actions: recording.actions,
  };
  return next;
}

module.exports = { loadSettings, getProfile, getStepDelayMs, getRecording, saveRecording };
