...
// A session row's clickable link text holds BOTH the date and the time, e.g.
// "چهارشنبه ۰۷ مرداد › ۱۹:۰۰" -- so when a single date has more than one session
// (a 19:00 and a 21:00 show), matching on the date alone is ambiguous and would
// pick whichever comes first. sessionTime, when given, is required to also be
// present in the same link's text, pinning the exact slot. It's an independent
// substring check (not "date › time"), so the caller never has to reproduce the
// exact separator/spacing tiwall renders between them.
async function getSessionState(page, sessionMatch, sessionTime) {
  return page.evaluate(
    ({ sessionMatch, sessionTime }) => {
      const links = Array.from(document.querySelectorAll('a[href="javascript:;"]'));
      const link = links.find((l) => {
        const t = l.textContent || '';
        return t.includes(sessionMatch) && (!sessionTime || t.includes(sessionTime));
      });
    ...
    { sessionMatch, sessionTime: sessionTime || null }
  );
}

async function clickSession(page, sessionMatch, sessionTime) {
  ...
  return page.evaluate(
    ({ sessionMatch, sessionTime }) => {
      ...
      for (const link of links) {
        ...
        const matches = text.includes(sessionMatch) && (!sessionTime || text.includes(sessionTime));
        if (matches && clickable) {
          link.click();
          return true;
        }
      }
...
async function clickBankPaymentMethod(page, timeoutMs = 8000) {
  try {
    await page.waitForFunction(
      () => {
        // document.body can briefly be null right after navigating to the bank's
        // page -- the new document is still being parsed when this predicate first
        // polls. Playwright treats a predicate that *throws* as a hard failure, not
        // "keep polling", so an unguarded document.body.innerText here kills the
        // whole wait on the very first poll instead of retrying until body exists.
        if (document.getElementById('walletRequestSubmit')) return true;
  ...
  return page.evaluate(() => {
    ...
    function findClickableContaining(text) {
      function deepestMatchAmong(selector) {
      ...
      // A status/heading line (e.g. "کیف پول زیربنا | موجودی: ...") can legitimately
      // contain the same wallet-name text as the real submit button below it, and
      // being earlier in the page it would otherwise win the match first -- clicking
      // it does nothing since it's not an actual control. Real actionable elements
      // (button/submit input) are checked first; only fall back to the broader
      // div/li/label/a search if none exists.
      return (
        deepestMatchAmong('button, input[type="submit"], input[type="button"]') ||
        deepestMatchAmong('a, label, [role="button"], div, li, input')
      );
    }

    // A genuinely non-interactive control -- unlike isDisabled(), this does NOT
    // reject on low opacity or pointer-events, which are visual/styling choices
    // (the wallet button is rendered a pale green by the gateway) rather than real
    // "you can't click this" signals. Only used when we already know exactly which
    // element we want (matched by its stable id), so second-guessing it visually
    // would just wrongly skip the right button.
    ...
    // The gateway's own "quick wallet" submit button has a stable id
    // (walletRequestSubmit) -- when the user is logged into the wallet, target it
    // directly. This is far more reliable than text matching, which kept resolving
    // to the wallet *balance* heading line above it that shares the same wording.
    // It only appears when a wallet session exists, so its presence is itself the
    // "user is logged in" signal. It's a real <button type=submit>, so clicking it
    // submits the wallet-payment form.
    const walletById = document.getElementById('walletRequestSubmit');
    if (walletById && !isHardDisabled(walletById)) {
      walletById.click();
